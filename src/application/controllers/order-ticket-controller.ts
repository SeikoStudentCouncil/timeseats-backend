...
export class OrderTicketController {
    ...
    async updatePaymentStatus(c: Context) {
        try {
            const id = c.req.param("id");
            const input = await c.req.json();
            const validatedData = OrderTicketValidator.validateUpdatePayment(input);
    ...
    async updateDeliveryStatus(c: Context) {
        try {
            const id = c.req.param("id");
            const input = await c.req.json();
            const validatedData = OrderTicketValidator.validateUpdateDelivery(input);
